...
async function checkAnkaraData() {
  ...
  try {
    ...
    // 3. Ankara mekanlarının ID'lerini al
    ...
    if (placeIds.length > 0) {
      // 4. Ankara mekanlarının yorumları (inArray kullan)
    ...
    const ankaraPlacesWithReviews = await db
      ...
      .orderBy(sql`review_count desc nulls last`)
      .limit(10)

    console.log('\n📝 En Çok Yorumlu Ankara Mekanları:')
    ...
    const lastUpdated = await db
      ...
      .orderBy(sql`updated_at desc nulls last`)
      .limit(1)

    if (lastUpdated.length > 0 && lastUpdated[0].updatedAt) {
      console.log(`\n🕐 Son Güncelleme: ${lastUpdated[0].name}`)
      console.log(`   Tarih: ${new Date(lastUpdated[0].updatedAt).toLocaleString('tr-TR')}`)
    }

    // 8. Toplam mekan sayısı (karşılaştırma için)
    ...
    // 9. Sync planı ile karşılaştırma
    console.log('\n📋 Sync Planı Karşılaştırması:')
    console.log('   Planlanan Bölgeler: 15 bölge')
    console.log('   Planlanan Kategoriler: 34 kategori')
    console.log('   Toplam Kombinasyon: 15 × 34 = 510 kombinasyon')
    console.log(`   Mevcut Ankara Mekanları: ${ankaraPlaces[0].count} mekan`)
